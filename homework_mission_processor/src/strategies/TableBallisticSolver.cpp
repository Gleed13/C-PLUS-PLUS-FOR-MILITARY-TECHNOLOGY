...
TableBallisticSolver::TableBallisticSolver(
...
std::optional<BallisticSolution> TableBallisticSolver::solve(
    ...
    const Ammo& ammo)
{
    ...
    const auto result = lookup(
        drone_config.altitude,
        drone_config.attackSpeed,
        ammo.mass,
        ammo.drag,
        ammo.lift);

    if (!result.has_value() ||
        result->fallTime <= 0.0F ||
        result->horizontalDistance <= 0.0F) {
...
bool TableBallisticSolver::load(const std::string& table_path)
{
    ...
    for (const std::size_t dimension : dimensions) {
        if (dimension < 2 ||
            !checkedMultiply(dimension, total_size)) {
            ERROR("Invalid ballistic table dimensions: " << table_path);
            return false;
        }
    }

    if (!readAxis(input, dimensions[0], altitude_axis_) ||
        !readAxis(input, dimensions[1], speed_axis_) ||
        !readAxis(input, dimensions[2], mass_axis_) ||
        !readAxis(input, dimensions[3], drag_axis_) ||
    ...
    for (TableResult& result : data_) {
        if (!(input >> result.fallTime >> result.horizontalDistance) ||
            !std::isfinite(result.fallTime) ||
...
std::optional<TableBallisticSolver::TableResult>
TableBallisticSolver::lookup(
    float altitude,
    float speed,
    float mass,
    float drag,
    float lift) const
{
    const std::array<float, kDimensionCount> query{
        altitude,
        speed,
        mass,
        drag,
    ...
    const std::array<const std::vector<float>*, kDimensionCount>
        axes{
            &altitude_axis_,
            &speed_axis_,
            &mass_axis_,
    ...
    for (std::size_t dimension = 0; dimension < kDimensionCount; ++dimension) {
        const auto axis_interpolation = findInterpolation(
            query.at(dimension),
            *axes.at(dimension));
        if (!axis_interpolation.has_value()) {
            return std::nullopt;
        }
        interpolation.at(dimension) =
            axis_interpolation.value();
    }
    ...
    for (std::size_t corner = 0; corner < kCornerCount; ++corner) {
        ...
        for (std::size_t dimension = 0;
             dimension < kDimensionCount;
             ++dimension) {
            indices.at(dimension) =
                interpolation.at(dimension).lowerIndex +
                ((corner >> dimension) & 1U);
        }

        values.at(corner) = data_.at(index(
            indices.at(0),
            indices.at(1),
            indices.at(2),
            indices.at(3),
    ...
    for (std::size_t dimension = 0; dimension < kDimensionCount; ++dimension) {
        for (std::size_t value_index = 0; value_index < active_values; value_index += 2) {
            values.at(value_index / 2) = interpolate(
                values.at(value_index),
                values.at(value_index + 1),
                interpolation.at(dimension).fraction);
        }
        active_values /= 2;
    }
...
{
    return ((((altitude_index * speed_axis_.size() + speed_index) *
                  mass_axis_.size() +
              mass_index) *
                 drag_axis_.size() +
             drag_index) *
...
std::optional<TableBallisticSolver::Interpolation>
TableBallisticSolver::findInterpolation(
    float value,
    const std::vector<float>& axis)
{
    ...
    const auto lower_index = static_cast<std::size_t>(
        std::distance(axis.begin(), upper) - 1);
    const float fraction =
        (value - axis[lower_index]) /
        (axis[lower_index + 1] - axis[lower_index]);
    return Interpolation{lower_index, fraction};
}

TableBallisticSolver::TableResult TableBallisticSolver::interpolate(
    const TableResult& lower,
    const TableResult& upper,
    float fraction)
{
    return {
      .fallTime = std::lerp(lower.fallTime, upper.fallTime, fraction),
      .horizontalDistance =
          std::lerp(lower.horizontalDistance,
                    upper.horizontalDistance,
                    fraction)
    };
}
    ...
    DropPoint& drop_point)
{
    const float distance_to_target = std::hypot(
        target_position.x - drone_position.x,
        target_position.y - drone_position.y);
    if (distance_to_target <= 0.0F) {
        ERROR("Non-positive distance to target: " << distance_to_target << " meters");
        return false;
    }

    const Coord target_offset =
        target_position - drone_position;
    const float ratio =
        (distance_to_target - horizontal_distance) /
        distance_to_target;
    if (horizontal_distance + acceleration_path >
        distance_to_target) {
        drop_point.intermPoint =
            target_position -
            target_offset *
    ...
    drop_point.firePoint =
        target_position - target_offset * ratio;
    return true;
}
