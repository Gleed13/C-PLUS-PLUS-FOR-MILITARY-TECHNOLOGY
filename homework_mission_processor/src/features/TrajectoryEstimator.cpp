    ...
    const DroneConfig& config) const
{
    ...
    if (drop_point.intermPoint.has_value()) {
        ...
        const float angle_to_intermediate = angleToTarget(
            estimated_drone.position,
            estimated_drone.direction,
            intermediate_point);
        ...
        if (std::abs(angle_to_intermediate) > config.turnThreshold) {
            ...
            estimated_drone.position.x +=
                remaining_deceleration_path *
                std::cos(estimated_drone.direction);
            estimated_drone.position.y +=
                remaining_deceleration_path *
                std::sin(estimated_drone.direction);
        ...
        const float distance_to_intermediate =
            distance(estimated_drone.position, intermediate_point);

        if (distance_to_intermediate <= remaining_deceleration_path) {
            total_time += estimated_drone.speed / acceleration;
        } else if (distance_to_intermediate >=
                   config.accelPath + remaining_acceleration_path) {
            const float distance_at_max_speed =
                distance_to_intermediate -
                remaining_acceleration_path -
                config.accelPath;
            total_time +=
                (2.0F * config.attackSpeed - estimated_drone.speed) /
                acceleration;
            ...
            const float acceleration_path_before_deceleration =
                std::max(
                    ...
                    (distance_to_intermediate -
                     remaining_deceleration_path) /
                    2.0F);
            const float peak_speed = std::sqrt(
                estimated_drone.speed * estimated_drone.speed +
                2.0F * acceleration *
                    acceleration_path_before_deceleration);
            total_time +=
                (2.0F * peak_speed - estimated_drone.speed) /
                acceleration;
        }
    ...
    const float angle_to_target = angleToTarget(
        estimated_drone.position,
        estimated_drone.direction,
        target_position);
    const float turn_threshold =
        estimated_drone.speed > 0.0F
            ? config.turnThreshold
            : config.turnThreshold / 2.0F;
    if (std::abs(angle_to_target) > turn_threshold) {
        total_time += std::abs(angle_to_target) / config.angularSpeed;
    }

    total_time +=
        (config.attackSpeed - estimated_drone.speed) /
        acceleration;
    ...
    total_time +=
        std::max(
            0.0F,
            distance_to_target - remaining_acceleration_path) /
        config.attackSpeed;
...
float TrajectoryEstimator::angleToTarget(
    ...
    const Coord& target_position)
{
    ...
    const float cross =
        current_direction.x * normalized_target.y -
        current_direction.y * normalized_target.x;
    const float dot =
        current_direction.x * normalized_target.x +
        current_direction.y * normalized_target.y;

    return std::atan2(cross, dot);
}

float TrajectoryEstimator::distance(
    const Coord& first,
    const Coord& second)
{
