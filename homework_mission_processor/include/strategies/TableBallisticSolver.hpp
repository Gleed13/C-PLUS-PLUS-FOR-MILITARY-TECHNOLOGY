...
private:
    ...
    std::optional<TableResult> lookup(float altitude, float speed, float mass, float drag, float lift) const;
    std::size_t index(
        std::size_t altitude_index, std::size_t speed_index, std::size_t mass_index, std::size_t drag_index, std::size_t lift_index) const;

    static std::optional<Interpolation> findInterpolation(float value, const std::vector<float>& axis);
    static TableResult interpolate(const TableResult& lower, const TableResult& upper, float fraction);
    static bool tryCalculateDropPoint(const Coord& drone_position,
