...
public:
    explicit DroneMovementController(const DroneMotionProfile& motion_profile);

    bool init(const DroneConfig& config);
    void reset();

    bool update(DroneState& drone, const DropPoint& destination);
...
private:
    ...
    static float angleToTarget(const DroneState& drone, const Coord& target_position);
    static void updatePosition(DroneState& drone, const DroneConfig& config);
};
