...
std::optional<float> TrajectoryEstimator::estimateArrivalTime(const DroneTelemetry& droneTelemetry,
                                                              ...
                                                              const DroneConfig& config) const
{
    ...
    DroneTelemetry estimated_drone_telemetry = droneTelemetry;
    float total_time = 0.0F;
    float remaining_acceleration_path = motion_profile_.remainingAccelerationPath(estimated_drone_telemetry.speed);
    float remaining_deceleration_path = motion_profile_.remainingDecelerationPath(estimated_drone_telemetry.speed);

    if (drop_point.intermPoint.has_value()) {
        const Coord intermediate_point = drop_point.intermPoint.value();
        const float angle_to_intermediate = angleToTarget(estimated_drone_telemetry.position, estimated_drone_telemetry.direction, intermediate_point);

        if (std::abs(angle_to_intermediate) > config.turnThreshold) {
            total_time += estimated_drone_telemetry.speed / acceleration;
            total_time += std::abs(angle_to_intermediate) / config.angularSpeed;

            estimated_drone_telemetry.direction += angle_to_intermediate;
            estimated_drone_telemetry.position.x += remaining_deceleration_path * std::cos(estimated_drone_telemetry.direction);
            estimated_drone_telemetry.position.y += remaining_deceleration_path * std::sin(estimated_drone_telemetry.direction);
        ...
        const float distance_to_intermediate = distance(estimated_drone_telemetry.position, intermediate_point);
        ...
        if (distance_to_intermediate <= remaining_deceleration_path) {
            ...
            total_time += (2.0F * config.attackSpeed - estimated_drone_telemetry.speed) / acceleration;
            ...
            const float peak_speed =
                std::sqrt(estimated_drone_telemetry.speed * estimated_drone_telemetry.speed + 2.0F * acceleration * acceleration_path_before_deceleration);
            total_time += (2.0F * peak_speed - estimated_drone_telemetry.speed) / acceleration;
        }

        estimated_drone_telemetry.position = intermediate_point;
        estimated_drone_telemetry.speed = 0.0F;
        estimated_drone_telemetry.status = DroneStatus::Stopped;
    }

    const float angle_to_target = angleToTarget(estimated_drone_telemetry.position, estimated_drone_telemetry.direction, target_position);
    const float turn_threshold = estimated_drone_telemetry.speed > 0.0F ? config.turnThreshold : config.turnThreshold / 2.0F;
    if (std::abs(angle_to_target) > turn_threshold) {
        total_time += std::abs(angle_to_target) / config.angularSpeed;
    }

    total_time += (config.attackSpeed - estimated_drone_telemetry.speed) / acceleration;

    remaining_acceleration_path = motion_profile_.remainingAccelerationPath(estimated_drone_telemetry.speed);
    const float distance_to_target = distance(estimated_drone_telemetry.position, target_position);
